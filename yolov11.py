...
def train_val_model(mode,data_path):
    
    
    if mode == 'train':
        # YOLO 모델 초기화 (학습 전: 초기 모델 경로, 학습 후: 현재 학습된 모델 경로)
        model = YOLO('D:\mobility_experiments\yolo11s.pt')
        # 모델 학습 수행
        results_train = model.train(
            ...
            val = False,         # 검증 수행 여부 (True 또는 False)
            exist_ok=True,      # 기존 모델 덮어쓰기 여부 (True 또는 False)
        )
    elif mode == 'val': 
        # YOLO 모델 초기화 (학습 전: 초기 모델 경로, 학습 후: 현재 학습된 모델 경로)
        model = YOLO('D:\\mobility_experiments\\train_results\\weights\\best.pt')
        # 모델 검증 수행
        results_val = model.val(
            data=data_path,         # 데이터셋 YAML 파일 경로
            split="val",           # 검증 데이터셋 분할 (e.g., "test")
            save_json=True,         # 검증 결과를 JSON 파일로 저장 (True 또는 False)
            project='D:\\mobility_experiments',       # 검증 결과 저장 디렉토리
        ...
        save_dir = "D:\\mobility_experiments\\validation_results"

        # 결과를 엑셀 파일로 저장
        with pd.ExcelWriter(save_dir + '/detection_metrics_results.xlsx') as writer:
...
def main():
    ...
    parser.add_argument('--val', action='store_true', help='모델 평가')
    ...
    if args.train:
        ...
        train_val_model(mode,args.data)
        print("[INFO] 학습이 완료되었습니다.")
    elif args.val:
        mode = 'val'
        print("[INFO] 검증을 시작합니다...")
        train_val_model(mode,args.data)
        print("[INFO] 검증이 완료되었습니다.")
        ...
        print("[WARNING] 실행할 작업이 지정되지 않았습니다. --train, --val, --test_cam 또는 --test_image를 사용하세요.")
